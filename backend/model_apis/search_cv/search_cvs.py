...
def open_files(path):
    ...
    formats_pdf = [".pdf"]
    formats_docx = [".docx"]
    files = glob.glob(path+"*.pdf")+glob.glob(path+"*.docx")
    files_names = []
...
def model_prediction(description,top_n,bag_of_words,vectorizer):
    ...
    files = glob.glob("cvs/"+"*.pdf")+glob.glob("cvs/"+"*.docx")+glob.glob("cvs/"+"*.doc")
    #print(files)
